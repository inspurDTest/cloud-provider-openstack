...
import (
	...
	v2monitors "github.com/inspurDTest/gophercloud/openstack/loadbalancer/v2/monitors"
	v2pools "github.com/inspurDTest/gophercloud/openstack/loadbalancer/v2/pools"
	neutrontags "github.com/inspurDTest/gophercloud/openstack/networking/v2/extensions/attributestags"
	...
	secgroups "github.com/inspurDTest/utils/openstack/networking/v2/extensions/security/groups"
...
const (
	...
	memeberFormat = "k8s_%s_%s_%s_%d_%d"
...
func (lbaas *LbaasV2) ensureOctaviaPool(portIndex int, lbID string, name string, listener *listeners.Listener, service *corev1.Service, port corev1.ServicePort, nodes []*corev1.Node, svcConf *serviceConfig, memberOpts map[int][]v2pools.BatchUpdateMemberOpts) (*v2pools.Pool, error) {
	pool, err := openstackutil.GetPoolByListener(lbaas.lb, lbID, listener.ID)
	if err != nil && err != cpoerrors.ErrNotFound {
...
func (lbaas *LbaasV2) ensureOctaviaLoadBalancer(ctx context.Context, clusterName string, service *corev1.Service, nodes []*corev1.Node, endpointSlices []*discoveryv1.EndpointSlice, lbID string) (lbs *corev1.LoadBalancerStatus, err error) {
	...
	if len(curListenerMapping) > len(service.Spec.Ports) {
		...
		for i := len(service.Spec.Ports); i < len(curListenerMapping); i++ {
			...
			if value, exists := curListenerMapping[name]; exists {
	...
	for portIndex, port := range service.Spec.Ports {
		...
		// 包含pool和memeber
		_, err = lbaas.ensureOctaviaPool(portIndex, loadbalancer.ID, cpoutil.Sprintf255(poolFormat, lbName, portIndex), listener, service, port, nodes, svcConf, lbmembers)
		if err != nil {
			return nil, err
		}
